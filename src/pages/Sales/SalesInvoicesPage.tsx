...
import { Search, Filter, Eye, FileText, Calendar, RefreshCw, XCircle, AlertTriangle } from 'lucide-react';
...
const SalesInvoicesPage = () => {
    ...
    const handleCancelClick = (invoice: SalesInvoice) => {
        setInvoiceToCancel(invoice);
        setCancelledByName(''); // Reset name
    ...
    const confirmCancelInvoice = async () => {
        ...
        try {
            await SalesInvoiceService.cancel(invoiceToCancel.id, cancelledByName);
            toast.success('تم إلغاء الفاتورة بنجاح');

            // Optimistically update the UI to show changes immediately
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-content-primary">فواتير المبيعات</h1>
                    <p className="text-content-secondary mt-1">سجل كامل لجميع عمليات البيع والفواتير الصادرة</p>
                </div>
            </div>
            ...
            <div className="bg-card p-4 rounded-xl shadow-sm border border-border flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full bg-canvas border border-border rounded-lg pl-3 pr-10 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 text-content-primary placeholder-content-muted transition-all"
                    />
                </div>
                ...
                <div className="flex gap-4">
                    <div className="relative min-w-[160px]">
                        ...
                        <select
                            ...
                            className="w-full bg-canvas border border-border rounded-lg pl-3 pr-10 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 text-content-primary appearance-none cursor-pointer"
                        >
                    ...
                    <button
                        ...
                        className="p-2.5 text-content-secondary hover:bg-canvas rounded-lg border border-border transition-colors hover:text-primary-600"
            ...
            <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                {isLoading && invoices.length === 0 ? (
                    <div className="p-12 text-center flex flex-col items-center justify-center">
                        ...
                        <p className="text-content-secondary">جاري تحميل الفواتير...</p>
                    </div>
                        ...
                        <table className="w-full text-right">
                            <thead className="bg-canvas/50 border-b border-border">
                                <tr>
                                    ...
                                    <th className="py-4 px-6 text-sm font-semibold text-content-secondary">العميل</th>
                            ...
                            <tbody className="divide-y divide-border">
                                {filteredInvoices.map((invoice) => (
                                    <tr key={invoice.id} className="group hover:bg-canvas/50 transition-colors">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="font-medium text-content-primary">{invoice.customer.name}</div>
                                            <div className="text-xs text-content-secondary font-mono mt-0.5">{invoice.customer.phone}</div>
                                        </td>
                                        ...
                                        <td className="py-4 px-6">
                                            <span className="font-bold text-content-primary">{invoice.totalAmount.toLocaleString()} ر.س</span>
                                        </td>
                                        <td className="py-4 px-6">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-primary-600 bg-primary-50 hover:bg-primary-100 dark:bg-primary-900/20 dark:hover:bg-primary-900/30 rounded-lg transition-colors"
                                                >
                                                    <Eye size={16} />
                                                    عرض
                                                </button>
                ...
                <div className="bg-canvas/30 px-6 py-4 border-t border-border flex justify-between items-center text-sm text-content-secondary">
                    ...
                    <span>الإجمالي الكلي: <span className="font-bold text-content-primary">{filteredInvoices.reduce((sum, inv) => sum + inv.totalAmount, 0).toLocaleString()} ر.س</span></span>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-card w-full max-w-md rounded-2xl shadow-xl border border-border p-6 animate-in zoom-in-95 duration-200">
                        ...
                        <p className="text-content-secondary mb-6">
                            هل أنت متأكد من رغبتك في إلغاء الفاتورة رقم <span className="font-mono font-bold text-content-primary mx-1">{invoiceToCancel?.invoiceNumber}</span>؟
